"""Unitree Go2 platform climb skill configuration."""

from skillsblender.tasks.path.config.platform_env_cfg import (
    PlatformPathEnvCfg,
    PLATFORM_TOP_HEIGHT,
    PIT_SLOPE_RANGE_FINAL,
    PIT_SLOPE_STEP,
    PIT_SIZE,
    PIT_PLATFORM_WIDTH,
    PIT_BORDER_WIDTH,
...
_CLIMB_INIT_H = PLATFORM_TOP_HEIGHT
_CLIMB_FINAL_H = 1.05
...
@configclass
class PlatformCurriculumCfg:
    """Platform-specific curriculum learning configuration."""

    climb_difficulty = CurrTerm(
        func=mdp.curriculum_platform_difficulty,
        params={
            ...
            "climb_len_step": 0.03,
            "pit_slope_min_final": PIT_SLOPE_RANGE_FINAL[0],
            "pit_slope_max_final": PIT_SLOPE_RANGE_FINAL[1],
            "pit_slope_step": PIT_SLOPE_STEP,
            "pit_size": PIT_SIZE,
            "pit_platform_width": PIT_PLATFORM_WIDTH,
            "pit_border_width": PIT_BORDER_WIDTH,
            "platform_top_height": PLATFORM_TOP_HEIGHT,
...
@configclass
class Go2PlatformEnvCfg(PlatformPathEnvCfg):
    """Unitree Go2 platform climb skill training configuration."""
    ...
    def __post_init__(self):
        ...
        if self.__class__.__name__ == "Go2PlatformEnvCfg":
            self.disable_zero_weight_rewards()


@configclass
class Go2PlatformCurEnvCfg(Go2PlatformEnvCfg):
    """Unitree Go2 platform training configuration with curriculum."""

    curriculum: PlatformCurriculumCfg = PlatformCurriculumCfg()

    def __post_init__(self):
        super().__post_init__()
        self.commands.path_tracking.platform_params.climb_height = _CLIMB_INIT_H
        self.commands.path_tracking.platform_params.climb_len = _CLIMB_INIT_LEN
        ...
        if self.__class__.__name__ == "Go2PlatformCurEnvCfg":
            self.disable_zero_weight_rewards()


@configclass
class Go2PlatformEnvCfg_PLAY(Go2PlatformEnvCfg):
    """Unitree Go2 platform PLAY configuration."""
...
@configclass
class Go2PlatformCurEnvCfg_PLAY(Go2PlatformCurEnvCfg):
    """Unitree Go2 platform curriculum PLAY configuration."""
