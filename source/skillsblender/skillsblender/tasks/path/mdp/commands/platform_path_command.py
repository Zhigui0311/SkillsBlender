...
class PlatformPathCommand(SegmentPathCommand):
    """Path with a PLATFORM climb segment (step-like rise + top traversal)."""
    ...
    def _resample_command(self, env_ids: torch.Tensor):
        ...
        cp = self.cfg.platform_params
        s_min, s_max = cp.start_dist_range
        min_len = float(s_max + cp.climb_len + 0.2)
        ...
        self._append_segment(env_ids, self.SKILL_ID["platform"], climb_s0, climb_s1, params=cl_params)
